...
git clone https://github.com/Liquid369/pictureex
cd pictureex/src
cd ..
prepare_system
create_swap
...
make install
if [ -d '/root/.pictureex' ] ; then
#Things to do
cd /root/.pictureex
rm -rf *
touch pictureex.conf
echo "rpcuser=user" >> pictureex.conf
echo "rpcpassword=pass123" >> pictureex.conf
echo "server=1" >> pictureex.conf
echo "daemon=1" >> pictureex.conf
echo "listen=1" >> pictureex.conf
echo "rpcallowip=127.0.0.1" >> pictureex.conf
echo "Finished setting up config,now starting daemon"
pictureexd
echo $endmessage
else
mkdir /root/.pictureex
cd /root/.pictureex
touch pictureex.conf
echo "rpcuser=user" >> pictureex.conf
echo "rpcpassword=pass123" >> pictureex.conf
