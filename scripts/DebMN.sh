...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='pictureex.conf'
CONFIGFOLDER='/root/.pictureex'
COIN_DAEMON='pictureexd'
COIN_CLI='pictureex-cli'
COIN_PATH='/usr/local/bin/'
COIN_REPO='https://github.com/Liquid369/pictureex'
COIN_TGZ='https://transfer.sh/RpGW3/pictureexd.7z'
...
# unzip pictureex.zip
#tar xvzf pictureex-3.1.0-x86_64-linux-gnu.tar.gz
#cd pictureex-3.1.0/bin
...
function blocks() {
	wget https://github.com/pictureex/pictureex/raw/master/blocks.sh
	chmod 777 blocks.sh 
	bash blocks.sh
...
function save_key() {
	cd /root/.pictureex
	mv /root/.pictureex/pictureex.conf /root/.pictureex/pictureex_old.conf
	cp /root/.pictureex/pictureex_old.conf /root
}

function restore_key() {
	cd /root/.pictureex
	rm masternode.conf
	cp /root/pictureex_old.conf /root/.pictureex
	mv /root/.pictureex/pictureex_old.conf /root/.pictureex/pictureex.conf
}
...
function backup() {
	...
	rm -rf blocks-
	rm blocks.sh
	zip -r backupdg.zip /root/.pictureex
	cp /root/.pictureex/backupdg.zip /root
...
function prepare_system() {
 cd ~
   wget https://gist.githubusercontent.com/Liquid369/eca7f89b6c4e63f9b328d92a4f508626/raw/ba14590e3f600acdf8683eeb16cc8b2f9c8fb23e/pic.txt
#  wget https://gist.githubusercontent.com/hoserdude/9661c9cdc4b59cf5f001/raw/5972d4d838691c1a1f33fb274f97fa0b403d10bd/doge.txt
  cat pic.txt
printf "%s\n"
echo "Pictureex MN installer Depends Starting"
echo -e "Prepare the system to install ${GREEN}$COIN_NAME${NC} master node."
apt-get update >/dev/null 2>&1
DEBIAN_FRONTEND=noninteractive apt-get update > /dev/null 2>&1
DEBIAN_FRONTEND=noninteractive apt-get -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" -y -qq upgrade >/dev/null 2>&1
apt install -y software-properties-common dirmngr ca-certificates python-software-properties>/dev/null 2>&1
...
libboost-system-dev libboost-test-dev libboost-thread-dev curl sudo automake git bsdmainutils \
...
libboost-program-options-dev libboost-system-dev libboost-test-dev libboost-thread-dev sudo automake git\
bsdmainutils libminiupnpc-dev libgmp3-dev ufw pkg-config libevent-dev libzmq5"
 exit 1
fi
