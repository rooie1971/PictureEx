...
public:
    ...
    bool WritezpicSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadzpicSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadzpicSeed_deprecated(uint256& seed);
    bool ErasezpicSeed();
    bool ErasezpicSeed_deprecated();

    bool WritezpicCount(const uint32_t& nCount);
    bool ReadzpicCount(uint32_t& nCount);
