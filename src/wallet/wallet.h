...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zpicspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 PIC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zpic send
enum ZerocoinSpendStatus {
    zpic_SPEND_OKAY = 0,                            // No error
    zpic_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    zpic_WALLET_LOCKED = 2,                         // Wallet was locked
    zpic_COMMIT_FAILED = 3,                         // Commit failed, reset status
    zpic_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    zpic_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    zpic_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    zpic_TRX_CREATE = 7,                            // Everything related to create the transaction
    zpic_TRX_CHANGE = 8,                            // Everything related to transaction change
    zpic_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    zpic_INVALID_COIN = 10,                         // Selected mint coin is not valid
    zpic_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    zpic_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    zpic_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    zpic_SPENT_USED_zpic = 14,                      // Coin has already been spend
    zpic_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    zpic_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void zpicBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreatezpicOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzpicAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzpicTracker> zpicTracker;
    ...
    void setZWallet(CzpicWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpicTracker = std::unique_ptr<CzpicTracker>(new CzpicTracker(strWalletFile));
    }

    CzpicWallet* getZWallet() { return zwalletMain; }
    ...
    void setzpicAutoBackups(bool fEnabled)
    {
    ...
    /// Get 5000PIC input and keys which can be used for the Masternode
    ...
    /** zpic reset */
    boost::signals2::signal<void()> NotifyzpicReset;
