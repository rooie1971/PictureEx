...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CzpicStake(spend));
    } else {
        ...
        CPICStake* PICInput = new CPICStake();
        PICInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(PICInput);
    }
