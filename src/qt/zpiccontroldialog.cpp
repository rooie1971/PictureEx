...
#include "ui_zpiccontroldialog.h"
...
std::set<std::string> zpicControlDialog::setSelectedMints;
std::set<CMintMeta> zpicControlDialog::setMints;

bool CzpicControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == zpicControlDialog::COLUMN_DENOMINATION || column == zpicControlDialog::COLUMN_VERSION || column == zpicControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


zpicControlDialog::zpicControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::zpicControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
zpicControlDialog::~zpicControlDialog()
{
    delete ui;
}

void zpicControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void zpicControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CzpicControlWidgetItem* itemDenom(new CzpicControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CzpicControlWidgetItem *itemMint = new CzpicControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zpicTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zpicTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zpic.";
            else if (!mint.isSeedCorrect)
                strReason = "The zpic seed used to mint this zpic is not the same as currently hold in the wallet";
            else
...
void zpicControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void zpicControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelzpic_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setzpicControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> zpicControlDialog::GetSelectedMints()
{
...
void zpicControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
