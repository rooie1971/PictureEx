...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::PIC);
    ...
    if (!settings.contains("nAnonymizepictureexAmount"))
        settings.setValue("nAnonymizepictureexAmount", 1000);

    nAnonymizepictureexAmount = settings.value("nAnonymizepictureexAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizepictureexAmount"))
        SoftSetArg("-anonymizepictureexamount", settings.value("nAnonymizepictureexAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in pictureex.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizepictureexAmount:
            return QVariant(nAnonymizepictureexAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizepictureexAmount:
            nAnonymizepictureexAmount = value.toInt();
            settings.setValue("nAnonymizepictureexAmount", nAnonymizepictureexAmount);
            emit anonymizepictureexAmountChanged(nAnonymizepictureexAmount);
            break;
        case CoinControlFeatures:
