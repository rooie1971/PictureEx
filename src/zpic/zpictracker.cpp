// Copyright (c) 2018 The pictureex developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zpic/deterministicmint.h>
#include "zpictracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzpicTracker::CzpicTracker(std::string strWalletFile)
{
...
CzpicTracker::~CzpicTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzpicTracker::Init()
{
...
bool CzpicTracker::Archive(CMintMeta& meta)
{
...
bool CzpicTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzpicTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzpicTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzpicTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzpicTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzpicTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzpicTracker::GetSerialHashes()
{
...
CAmount CzpicTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzpicTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzpicTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzpicTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzpicTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzpicTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzpicTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzpicTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzpicTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzpicTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzpicTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzpicWallet* zpicWallet)
{
    bool iszpicWalletInitialized = (NULL != zpicWallet);
    CMintMeta meta;
    ...
    if (! iszpicWalletInitialized)
        zpicWallet = new CzpicWallet(strWalletFile);
    meta.isSeedCorrect = zpicWallet->CheckSeed(dMint);
    if (! iszpicWalletInitialized)
        delete zpicWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzpicTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzpicTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzpicTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzpicTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzpicTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzpicTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzpicWallet* zpicWallet = new CzpicWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zpicWallet);
        }
        delete zpicWallet;
        LogPrint("zero", "%s: added %d dzpic from DB\n", __func__, listDeterministicDB.size());
    }
