...
CzpicWallet::CzpicWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zpic seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadzpicSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.ErasezpicSeed_deprecated()) {
                    LogPrintf("%s: Updated zpic seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zpic seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zpic wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzpicWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WritezpicCount(nCountLastUsed);
    else if (!walletdb.ReadzpicCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzpicWallet::Lock()
{
    seedMaster = 0;
}

void CzpicWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzpicWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedTozpic(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzpicWallet::LoadMintPoolFromDB()
{
...
void CzpicWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzpicWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzpicWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zpicTracker->HasPubcoinHash(pMint.first)) {
...
bool CzpicWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedTozpic(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zpicTracker which also adds to database
    pwalletMain->zpicTracker->Add(dMint, true);
...
void CzpicWallet::SeedTozpic(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzpicWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzpicWallet::GenerateDeterministiczpic(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzpicWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzpicWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzpicWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
